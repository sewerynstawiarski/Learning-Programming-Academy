...
public class Main {
    public static void main(String[] args) {
        ...
        Function<String,String> lastName = s -> s.concat(" Brzęczyszczykiewicz");
        ...
        Function<String, String[]> f0 = uCase.andThen(s -> s.concat(" Psiajucha"))
                .andThen(s -> s.split(" "));
        System.out.println(Arrays.toString(f0.apply(name)));

        Function<String, String> f1 = uCase.andThen(s -> s.concat(" Psiajucha"))
                .andThen(s -> s.split(" "))
                .andThen(s -> s[1].toUpperCase() + ", " + s[0]);

        System.out.println(f1.apply(name));

        Function<String, Integer> f2 = uCase.andThen(s -> s.concat(" Psiajucha"))
                .andThen(s -> s.split(" "))
        ...
        String[] otherNames = {"Anna", "Bob", "Carol"};
        Consumer<String> s0 = s -> System.out.print(s.charAt(0));
        Consumer<String> s1 = System.out::println;
        Arrays.asList(otherNames).forEach(s0.andThen(s -> System.out.print(" - ")).andThen(s1));
